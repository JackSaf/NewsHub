...
object ArticleNetworkEntityMapper : EntityMapper<ArticleNetworkEntity, Article> {
    ...
    override fun domainModelToEntity(domainModel: Article): ArticleNetworkEntity =
        ArticleNetworkEntity(
            domainModel.author,
            domainModel.content,
    ...
    override fun entityListToDomainModelList(entityList: List<ArticleNetworkEntity>): List<Article> =
        entityList.map { entityToDomainModel(it) }

    override fun domainModelListToEntityList(domainModelList: List<Article>): List<ArticleNetworkEntity> =
